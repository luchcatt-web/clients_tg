...
async def main():
    ...
    # Первичная проверка записей
    print("\n🔍 Первичная проверка записей...")
    await reminder_scheduler.run_once()
    
    print("\n" + "=" * 50)
    print("✅ Система запущена и готова к работе!")
    print("=" * 50)
    print(f"\n📍 Webhook URL: http://{config.WEBHOOK_HOST}:{config.WEBHOOK_PORT}/webhook/yclients")
    print("   (Укажите этот URL в настройках YClients для получения событий)")
    print("\n🔄 Напоминания проверяются каждые 5 минут")
    print("   - За 24 часа до визита")
    print("   - За 2 часа до визита")
